...
public class SafeProperties extends java.util.Properties  {
   ...
   public synchronized void load(InputStream inStream) throws IOException {
      ...
      in = new BufferedReader(new InputStreamReader(inStream, code));
      while (true) {
         // Get next line
   ...
   public synchronized void store(OutputStream out, String header) throws IOException {
      ...
      awriter = new BufferedWriter(new OutputStreamWriter(out,code));
      if (header != null)
         writeln(awriter, "#" + header);
      List entrys = context.getCommentOrEntrys();
      /*for (int i = 0; i < entrys.size(); i++) {
         Object obj = entrys.get(i);
   ...
   public synchronized Object put(Object key, Object value, String line,String comment) {
      context.putOrUpdate(key.toString(), value.toString(), line,comment);
      return super.put(key, value);
   }
   ...
   class PropertiesContext {
      ...
      public void putOrUpdate(String key, String value, String line) {
         PropertyEntry pe = new PropertyEntry(key, value, line,line);
         remove(key);
         commentOrEntrys.add(pe);
      }
      public void putOrUpdate(String key, String value, String line,String comment) {
         PropertyEntry pe = new PropertyEntry(key, value, null,comment);
         int index = removeComment(key);
         commentOrEntrys.add(index,"#"+comment);
         commentOrEntrys.add(index+1,pe);
      ...
      public int removeComment(String key) {
         for (int index = 0; index < commentOrEntrys.size(); index++) {
            Object obj = commentOrEntrys.get(index);
            if (obj instanceof PropertyEntry) {
               if (obj != null) {
                  if (key.equals(((PropertyEntry) obj).getKey())) {
                     commentOrEntrys.remove(obj);
                     if (commentOrEntrys.get(index-1).toString().startsWith("#")){
                        commentOrEntrys.remove(index-1);
                        return index-1;
                     }
                     return index;
                  }
